...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchSelectedDetails = async () => {
      ...
      try {
        ...
        const isAnalyzed =
          resumeData.score && resumeData.score.overall !== undefined;
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: content + hint },
      ]);
    } catch (err) {
      console.error(err);
      setMessages((prev) => [
  ...
  const triggerQuickPrompt = (promptType) => {
    ...
    const role =
      selectedResume.recommendations?.targetRole || "Software Engineer";
    let promptText = "";
    ...
    return parts.map((part, idx) => {
      if (part.startsWith("```") && part.endsWith("```")) {
        ...
        return (
          <div
            key={idx}
            className="my-3 rounded-xl overflow-hidden border border-slate-200/40 dark:border-slate-800/40 bg-slate-950 text-slate-100 font-mono text-xs"
          >
          ...
          {lines.map((line, lineIdx) => {
            ...
            if (line.startsWith("### ")) {
              return (
                <h4
                  key={lineIdx}
                  className="text-sm font-bold text-slate-900 dark:text-white mt-4 mb-1"
                >
                  {line.replace("### ", "")}
                </h4>
              );
            ...
            if (line.startsWith("## ")) {
              return (
                <h3
                  key={lineIdx}
                  className="text-base font-bold text-slate-900 dark:text-white mt-5 mb-2"
                >
                  {line.replace("## ", "")}
                </h3>
              );
              ...
              return codeParts.map((cp, cpIdx) => {
                if (cp.startsWith("`") && cp.endsWith("`")) {
                  return (
                    <code
                      key={cpIdx}
                      className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-mono text-[11px] font-semibold"
                    >
                      {cp.slice(1, -1)}
                    </code>
                  );
            ...
            if (isBullet) {
              return (
                <div key={lineIdx} className="flex items-start gap-2 pl-3">
                  <span className="text-indigo-500 dark:text-indigo-400 mt-1.5 shrink-0 select-none">
                    •
                  </span>
          ...
          <div className="glass-panel border border-slate-200/50 dark:border-slate-800/40 p-5 rounded-2xl flex flex-col gap-4 shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Select a profile to load into RAG context.
              </p>
            </div>
            ...
            <div className="glass-panel border border-slate-200/50 dark:border-slate-800/40 p-5 rounded-2xl shadow-sm flex flex-col gap-4">
              ...
              {isAnalyzed ? (
                <div className="flex flex-col gap-4">
                  ...
                  {selectedResume.recommendations.missingSkills &&
                    selectedResume.recommendations.missingSkills.length > 0 && (
                      <div className="flex flex-col gap-1.5">
                        <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">
                          Missing Skills (click to ask)
                        </span>
                          ...
                          {selectedResume.recommendations.missingSkills
                            ...
                            .map((skill, idx) => (
                              <button
                                ...
                                onClick={() =>
                                  handleSend(
                                    `How can I master ${skill} and build projects with it?`,
                                  )
                                }
        ...
        <div className="flex-1 flex flex-col glass-panel border border-slate-200/50 dark:border-slate-800/40 rounded-3xl shadow-sm overflow-hidden min-h-[500px]">
          ...
          <div className="px-6 py-4 border-b border-slate-200/50 dark:border-slate-800/40 flex items-center justify-between bg-white/40 dark:bg-slate-900/20 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-display font-extrabold text-sm text-slate-800 dark:text-white">
                  AI Career Assistant
                </h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 max-h-[calc(100vh-21rem)] md:max-h-[calc(100vh-17rem)]">
            {messages.map((m, idx) => (
              ...
              >
                {/* Avatar */}
                <div
                  className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 border text-xs
                  ${
                    m.role === "user"
                ...
                {/* Text Bubble */}
                <div
                  className={`p-4 rounded-2xl text-sm border transition-all duration-200 shadow-sm
                  ${
                    m.role === "user"
